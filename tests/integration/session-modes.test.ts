...
describe('Session Mode Comparison - Integration Test', () => {
  ...
  describe('Cross-Mode User Experience Integration', () => {
    ...
    it('should persist mode comparison preferences for guest users', async () => {
      ...
      // Verify persistence
      const storedPrefs = JSON.parse(localStorage.getItem('zen-focus-guest-mode-preferences') || '{}');
      ...
      const popularMode = Object.keys(storedPrefs.modeUsageCount)
        .reduce((a, b) => storedPrefs.modeUsageCount[a] > storedPrefs.modeUsageCount[b] ? a : b);
